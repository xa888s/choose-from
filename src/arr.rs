...
pub struct SelectorArr<const N: usize, T> {
    choices: [T; N],
}

impl<const N: usize, T> SelectorArr<N, T> {
    pub(crate) fn with_choices(choices: [T; N]) -> SelectorArr<N, T> {
        SelectorArr { choices }
    }
    ...
    ///
    /// let chosen = choose_from_fixed(choices).with(|[first, second, third]| {
    ...
    /// Like [with](SelectorArr::with), but for returning any number of chosen values. Use this when
    ...
    /// let choices = ["Hi", "how", "are ya?"];
    ///
    /// let chosen = choose_from_fixed(choices).any_with(|choices| {
